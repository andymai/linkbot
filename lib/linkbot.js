...
LinkBot.prototype._onMessage = function(message) {
	if (this._isChatMessage(message) && this._isDotCommand(message)) {
...
LinkBot.prototype._parseBuffer = function(message) {
	...
	switch (c) {
		case 'bookmark':
			...
			if (handle == '')
				self.postMessage(message.channel, 'Nothing to bookmark. Usage: .bookmark <keyword> <data>', {
					as_user: true
				});
			else this._addLink(handle, message, message.channel);
			break;
		...
		case 'unmark':
			...
			else this._removeLink(handle, message.channel);
			break;
...
LinkBot.prototype._fetchBookmark = function(handle) {
	...
	self.db.get(
		'SELECT link FROM bookmarks WHERE handle = $handle AND ACTIVE = 1 LIMIT 1',
		{ $handle: handle },
...
LinkBot.prototype._getWeather = function(zipcode, channel) {
	...
	weather.find(
		...
		function(err, result) {
			if (err) {
				console.log(err);
				return self.postMessage(channel, 'Unable to fetch weather. Usage: .weather <zipcode>', {
					as_user: true
				});
			}
			...
			var output =
				obj[0].location.name +
				' — High: ' +
				obj[0].forecast[0].high +
				' Low: ' +
				obj[0].forecast[0].low +
				': Current: ' +
				obj[0].current.temperature;
